...
class sparseIndexSet {
  ...
  * 	   To determine the weights, we have to check for each multi index alpha
  * 	   contained in _alpha if alpha + beta is in _alpha for each beta in
  * 	   {0,1}^{n}. If this is the case, we update the weight by (-1)^|beta|.
  * 	   Of course, the condition is always fulfilled for the multiindex
  * 	   beta = 0. Therefore, we skip this multiindex and initialize the
  *      weight cw with 1 which is associated with the second variable in the 
  *      function call to cw_alpha.
  ...
  */
  ...
  void comp_indexSet(int q, int dim, const Compare &cpFun) {
    int k = 0;
    ...
    if (cpFun(currInd) <= _q) {
      // check if _cw(0)\neq 0
      if (cpFun(_myOnes) > _q)
        _cw(0) = sparseIndexSet::cw_alpha(0, 1, 1, currInd, cpFun);
      if (_cw(0)) ++k;
      // compute all other indices in Yw_alpha recursively
      sparseIndexSet::Yw_alpha(0, &k, cpFun, currInd);
    }
  ...
  */
  template <class Compare>
  void Yw_alpha(int maxBit, int *k, const Compare &cpFun,
    ...
    for (int i = maxBit; i < _dim; ++i) {
      ...
      if (cpFun(currInd) <= _q) {
        ...
        if (cpFun(currInd + _myOnes) > _q)
          cw = sparseIndexSet::cw_alpha(0, 1, 1, currInd, cpFun);
        else
          cw = 0;
  ...
  *	   \param[in] maxBit position of the digit of the current multiindex
  *such
  *                 that only digits with position greater to maxBit are
  ...
  *(-1)^|beta|.
  *	   Hence, in order to determine the value of the coefficient, we have to
  *      check for each beta in {0,1}^_dim if alpha+beta belongs to the index
  *set
  *      _alpha. For beta=(0,...,0), alpha+beta belongs to _alpha and, hence, we
  ...
  *Yw_alpha.
  *	   The only difference of the tree is that once a digit of the
  *multiindex
  *	   alpha is modified, we do not have to consider further modification of
  *	   this digit due to the condition beta in {0,1}^_dim. This is realized
  *	   in the recursive algorithm consider only digits which are greater
  *than
  *      maxBit in contrast to the "greater than and equal" condition in
  *Yw_alpha.
  ...
  *      of the tree.
  *	   Of course, this modification has to be revoked when the recursion
  *goes
  *	   back to the parent node in the tree.
  ...
  *this
  *	   would be the correct value cw(1,0)=-1.
  *
  */
  template <class Compare>
  int cw_alpha(int maxBit, int cw, int lvl, Eigen::VectorXi &ind,
               const Compare &cpFun) {
    ...
    for (int i = maxBit; i < _dim; ++i) {
      ...
      if (cpFun(ind) <= _q) {
        ...
        cw = sparseIndexSet::cw_alpha(i + 1, cw, lvl + 1, ind, cpFun);
      }
      --ind(i);
    }
